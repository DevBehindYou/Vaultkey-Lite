    ...
    Column(
        ...
        Button(
            ...
            onClick = {
                if (isFirstRun) {
                    if (password.length < 8) {
                        ...
                        // PBKDF2 (210k iterations) + opening SQLCipher are seconds
                        // of work — run them off the main thread to avoid an ANR.
                        loading = true
                        scope.launch {
                            withContext(Dispatchers.Default) { session.setUpNewVault(password.toCharArray()) }
                            loading = false
                    ...
                    scope.launch {
                        val unlocked = withContext(Dispatchers.Default) {
                            session.unlockWithPassword(password.toCharArray())
                        }
                        loading = false
                        if (unlocked) {
                            onUnlocked()
            ...
            Text(
                ...
                else if (isFirstRun) "Create vault"
                else "Unlock"
            )
        }

 ...
 * iv+ciphertext produced by FieldCipher (the live session key) — never plaintext,
 * even in this DB, even though the whole SQLite file is itself SQLCipher-encrypted
 * at rest (defense in depth: DB-file compromise still doesn't yield plaintext secrets).
 */
...
@Entity(
    ...
    // Index the FK column: the lookup joins on it, and Room warns about full
    // table scans on parent updates/deletes without it.
